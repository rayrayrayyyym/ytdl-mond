...
then
    ...
    ./yt-dlp_linux --live-from-start $URL -P "$dlDirectory" --merge-output-format mp4 -o "%(upload_date>%Y)s/%(upload_date>%Y-%m-%d)s %(title)s.%(ext)s" --sleep-requests 5 --sleep-interval 2 --max-sleep-interval 30 --trim-filenames 70
fi
